...
model_name = 'lymph_nodes_model_2_6x.h5'

train_path="./DATA_FINAL/2.5D-II/train/"
test_path="./DATA_FINAL/2.5D-II/test/"
...
for train, test in kfold.split(ids):
    ...
    np.save("./SVM1/2.5D-II/training_"+str(cnt)+".npy", svm_training)
    np.save("./SVM1/2.5D-II/testing_"+str(cnt)+".npy", svm_testing)'''

plt.legend(loc=0)
plt.savefig("./ROC_CURVES/2_small_noTTA_please")
plt.close()
